...
{
    public class PAC : Models.Indicator
    {
        /* Indicator Values */
            ...
            Values = new()
                {
                    ...
                    { "TOC", TopOfChannel.Select(x => (object?)x.Smma ?? 0).ToList().GetRange(data.Count-25,25) },
                    { "BOC", BottomOfChannel.Select(x => (object?)x.Smma ?? 0).ToList().GetRange(data.Count-25,25) }
                };
            //Call update action
