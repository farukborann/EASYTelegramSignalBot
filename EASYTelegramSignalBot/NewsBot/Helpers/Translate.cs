...
namespace EASYTelegramSignalBot.NewsBot.Helpers
{
    public static class Translate
    {
        public static async Task<string?> TranslateText(string from, string to, string data)
        {
            ...
            try
            {
                HttpResponseMessage? result = Task.Run(() => client.GetAsync(new Uri("https://translate.googleapis.com/translate_a/single?client=gtx&sl=" + from + "&tl=" + to + "&dt=t&q=" + data))).Result;
                string[] json = result.Content.ReadAsStringAsync().Result.Split('"');
                string? translatedText = json[1];
