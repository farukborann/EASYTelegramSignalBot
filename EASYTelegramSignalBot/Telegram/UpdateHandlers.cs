using Telegram.Bot;
using Telegram.Bot.Types;
using Telegram.Bot.Exceptions;
using Telegram.Bot.Types.Enums;
using Telegram.Bot.Types.ReplyMarkups;
using System.Threading.Tasks;
using System;
using System.Linq;
using System.Threading;

namespace EASYTelegramSignalBot.Telegram
{
    public static class UpdateHandlers
    {
        //Main Methods
        public static Task PollingErrorHandler(ITelegramBotClient botClient, Exception exception, CancellationToken cancellationToken)
        {
            var ErrorMessage = exception switch
            {
                ApiRequestException apiRequestException => $"Telegram API Error:\n[{apiRequestException.ErrorCode}]\n{apiRequestException.Message}",
                _ => exception.ToString()
            };

            Console.WriteLine(ErrorMessage);
            return Task.CompletedTask;
        }

        public static async Task HandleUpdateAsync(ITelegramBotClient botClient, Update update, CancellationToken cancellationToken)
        {
            var handler = update.Type switch
            {
                UpdateType.Message => BotOnMessageReceived(botClient, update.Message!),
            };

            try
            {
                await handler;
            }
            catch (Exception exception)
            {
                await PollingErrorHandler(botClient, exception, cancellationToken);
            }
        }

        //New Message Or Callback Handled Methods
        private static Task BotOnMessageReceived(ITelegramBotClient botClient, Message message)
        {
            if (message.Text is not { } messageText)
                return Task.Delay(0);

            if (messageText == "/start") DefaultMessage(botClient, message);

            return Task.Delay(0);
        }

        //Answer Methods
        static async Task DefaultMessage(ITelegramBotClient botClient, Message message)
        {
            string text = "";
            if (string.IsNullOrEmpty(message.Chat.Username))
            {
                text = "Lütfen önce kullanıcı adı belirleyin.\nhttps://telegram.org/faq#s-kullanici-adlari-nedir-nasil-alabilirim";
            }
            else if (Database.Connection.Context.Users.Any(x => x.UserName == message.Chat.Username))
            {
                var user = Database.Connection.Context.Users.Where(x => x.UserName == message.Chat.Username).First();

                if(user.ChatId == 0)
                {
                    user.ChatId = message.Chat.Id;
                    Database.Connection.Context.SaveChanges();
                }

                if(user.TDIExpiryDate < DateTime.Now)
                {
                    text = "TDI Üyeliğiniz bitmiştir. Lütfen yenileyiniz.";
                }
                else
                {
                    text = $"Hoşgeldiniz!";
                    if (user.TDI) text += $"\nTDI Bot üyeliğiniz {user.TDIExpiryDate} tarihinde sona erecektir.";
                    if (user.PAC) text += $"\nPAC Bot üyeliğiniz {user.PACExpiryDate} tarihinde sona erecektir.";
                    text += "Bizi tercih ettiğiniz için teşekkürler.";
                }
            }
            else
            {
                text = "Üyeliğiniz bulunamamıştır. Lütfen bizimle iletişime geçin.";
            }

            await botClient.SendTextMessageAsync(chatId: message.Chat.Id, parseMode: ParseMode.Markdown,
                                                        text: text);
        }
    }
}